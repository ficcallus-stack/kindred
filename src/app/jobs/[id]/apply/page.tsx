...
const JOB = {
  ...
  tags: ["2 Children", "Infant & Toddler", "Mon-Fri (30hrs)"],
  image: "https://lh3.googleusercontent.com/aida-public/AB6AXuBZqcY8r21R3C_cQ6pKOfLQdYENKk_pHSl0jowLuVWWf-zmCWwB_wouilMllT4GoHuLX4FUWUqcYPkWKEKhrmZoLNL-05BqbwADiT9NfQPvzmSsR7v0SAGYMCvNotxeuGcPGadInEDl0jkvbV-b2-vb-_52Mt1C9uClrjGt3ul3_wc9QphzXK4bwabRT3glavtrPp-ekIrT7oHUsL6rORO3M3bRCYccS1_hvCTCMsPyL32-BiAv-MqtrFu5VWHXwoRVFajCp6opB9A"
};
    ...
    <div className="bg-surface min-h-screen pb-32">
      ...
      <main className="pt-24 max-w-7xl mx-auto flex flex-col md:flex-row gap-8 px-6 pb-12">
        ...
        <aside className="h-fit sticky top-24 hidden md:flex flex-col gap-2 p-6 bg-surface-container-low/50 backdrop-blur-md rounded-[2rem] border border-outline-variant/5 w-72">
          ...
          <button className="mt-12 py-4 px-6 rounded-2xl border-2 border-primary/10 text-primary font-black uppercase tracking-widest text-[10px] hover:bg-primary hover:text-white transition-all active:scale-95">
            Save Draft
          </button>
        </aside>
        ...
        <section className="flex-grow space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
          ...
          {/* Job Summary Card */}
          <div className="bg-surface-container-lowest p-2 rounded-[2.5rem] shadow-sm overflow-hidden flex flex-col sm:flex-row relative group hover:shadow-2xl transition-shadow">
            <div className="sm:w-1/3 aspect-[4/3] sm:aspect-auto overflow-hidden rounded-[2rem]">
              <img 
                src={JOB.image} 
                alt="Job" 
            ...
            <div className="p-10 sm:w-2/3 flex flex-col justify-center space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] text-secondary mb-2 block">{JOB.type}</span>
                  <h1 className="text-4xl font-black font-headline text-primary tracking-tighter leading-none">{JOB.family}</h1>
                  <p className="text-on-surface-variant flex items-center gap-2 text-sm font-black uppercase tracking-widest opacity-40 mt-3">
                    <MaterialIcon name="location_on" className="text-secondary" />
                    {JOB.location}
                  </p>
                </div>
                <div className="text-right">
                  <div className="text-3xl font-black text-primary italic tracking-tighter">{JOB.rate}</div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant opacity-40">per hour</div>
                </div>
              </div>
              <div className="flex flex-wrap gap-2 pt-4">
                {JOB.tags.map(tag => (
                  <span key={tag} className="px-4 py-2 bg-tertiary-fixed/30 text-on-tertiary-fixed text-[10px] font-black uppercase tracking-widest rounded-xl">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-surface-container-low rounded-[3rem] p-10 md:p-12 shadow-inner border border-white/50 relative overflow-hidden group">
              ...
              <p className="text-on-surface-variant text-sm font-medium opacity-60 mb-10 italic">Select the skills that match the Thompson's requirements.</p>
            ...
            <div className="bg-surface-container-low rounded-[3rem] p-10 md:p-12 shadow-inner border border-white/50 relative overflow-hidden group">
              <div className="flex justify-between items-baseline mb-10 px-2">
                ...
                <div className="px-5 py-2 bg-secondary/10 text-secondary text-[10px] font-black uppercase tracking-widest rounded-full">30 hrs / week</div>
              </div>
              ...
              <div className="mt-10 flex items-center gap-3 px-6 py-4 bg-white/50 rounded-2xl border border-primary/5 text-on-surface-variant text-[10px] font-black uppercase tracking-widest opacity-60">
                <MaterialIcon name="check_circle" className="text-primary text-xl" fill />
                <span>I confirm availability for these hours.</span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-6 pt-12 border-t border-outline-variant/5">
              <Link 
                ...
                className="flex-grow py-6 bg-gradient-to-br from-primary to-primary-container text-white rounded-[2rem] font-black uppercase tracking-[0.2em] text-xs shadow-[0_20px_40px_-10px_rgba(3,31,65,0.4)] hover:-translate-y-1 hover:shadow-[0_30px_60px_-15px_rgba(3,31,65,0.5)] transition-all flex items-center justify-center gap-4 group"
              >
                Submit Application
                <MaterialIcon name="rocket_launch" className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
              </Link>
              <button className="sm:w-1/3 py-6 bg-slate-100 text-slate-900 font-black uppercase tracking-[0.2em] text-xs rounded-[2rem] border-2 border-transparent hover:border-primary/10 transition-all active:scale-95">
                Save Draft
              </button>
            </div>
          </div>
        ...
        <aside className="md:w-80 space-y-10">
          ...
          <div className="bg-surface-container-low p-10 rounded-[3rem] shadow-sm border border-outline-variant/10 group hover:shadow-2xl transition-all">
            ...
            <ul className="space-y-8">
              {[
                ...
                { title: "Highlight Relevance", desc: "Focus on experience that matches their children's ages (Infant & Toddler)." },
          ...
          <div className="bg-primary p-10 rounded-[3rem] relative overflow-hidden group shadow-2xl shadow-primary/40">
            ...
            <div className="relative z-10 space-y-4">
              <h4 className="text-white text-2xl font-black italic tracking-tighter">Need Help?</h4>
              <p className="text-on-primary-container text-xs font-bold font-body opacity-80 leading-relaxed uppercase tracking-widest">Our concierge team is here to refine your story.</p>
              <a className="text-secondary-fixed-dim text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-3 hover:translate-x-2 transition-transform cursor-pointer">
                Contact Support
